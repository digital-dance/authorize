package com.digital.dance.common.controller;
...
import com.digital.dance.common.utils.vcode.Captcha;
...
//import com.digital.dance.user.manager.UserManagerService;
...
import com.digital.dance.common.utils.vcode.Captcha;
import com.digital.dance.common.utils.vcode.GifCaptcha;
import com.digital.dance.common.utils.vcode.SpecCaptcha;

//import com.digital.dance.permission.service.URoleService;
...
//	URoleService uRoleService;
	@Autowired
	VerifyCodeUtilsService verifyCodeUtilsService;
//	@Autowired
...
//		uRoleService.initData();
	...
	public void getVCode(HttpServletResponse response,HttpServletRequest request){
		try {
	        ...
	        //生成随机字串  
	        String verifyCode = verifyCodeUtilsService.generateVerifyCode(4);
	        //存入Shiro会话session
			SSOLoginFilter.save2Session(request, verifyCodeUtilsService.V_CODE, verifyCode.toLowerCase());
	        //tokenManagerService.setVal2Session(verifyCodeUtilsService.V_CODE, verifyCode.toLowerCase());
	        //生成图片  
	        int w = 146, h = 33;  
	        verifyCodeUtilsService.outputImage(w, h, response.getOutputStream(), verifyCode);
		} catch (Exception e) {
	...
	public void getGifCode(HttpServletResponse response,HttpServletRequest request){
		try {
	       ...
	       //存入Shiro会话session  
			...
			SSOLoginFilter.save2Session(request, verifyCodeUtilsService.V_CODE, captcha.text().toLowerCase());
//	        tokenManagerService.setVal2Session(verifyCodeUtilsService.V_CODE, captcha.text().toLowerCase());
		} catch (Exception e) {
			LoggerUtils.fmtError(getClass(),e, "获取验证码异常：%s",e.getMessage());
		}
	}
